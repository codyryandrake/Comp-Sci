...
public class KetchupIF {
	...
	public static void main(String[] args) throws IOException
	{
		...
		while (input.hasNext()) //If the document still has content, the loop continues.
		{
			...
			if (manufacturer.equals("heinz"))
			{
				profitHeinz +=pHeinz;
				bottlesHeinz += 1;
			}
			else if (manufacturer.equals("hunts"))
			{
				profitHunts +=pHunts;
				bottlesHunts += 1;
			}
			else if (manufacturer.equals("delmonte"))
			{
				profitDelmonte +=pDelmonte;
				bottlesDelmonte += 1;
			}
			else if (manufacturer.equals("stb"))
			{
				profitSTB +=pSTB;
				bottlesSTB += 1;
			}
